...
### Current State
Phase 0 (Foundation) is complete: Nx monorepo, shared packages (`shared`, `config`, `logger`, `events`, `testing`, `database`), Docker Compose local infra, the gateway, and a GitHub Actions CI pipeline (typecheck/lint/test/build via `nx affected` + dependency/secret scanning). The Ledger Service (`apps/ledger-service`) implements double-entry accounting — journal posting, balance projections, and financial correctness tests (14 tests). The Wallet Service (`apps/wallet-service`) implements wallet lifecycle, reservations, and optimistic locking, with balance projections now synced **event-driven** from the Ledger via Pub/Sub (financial-correctness tests: 36). The Transfer Service (`apps/transfer-service`) orchestrates money movement as a Saga (reserve → ledger post → capture) with compensation, idempotency, and status-history audit trail (15 tests).
