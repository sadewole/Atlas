...
### Current State
Phase 0 (Foundation) is complete: Nx monorepo, shared packages (`shared`, `config`, `logger`, `events`, `testing`, `database`), Docker Compose local infra, the gateway, and a GitHub Actions CI pipeline (typecheck/lint/test/build via `nx affected` + dependency/secret scanning). The Ledger Service (`apps/ledger-service`) implements double-entry accounting with an **outbox pattern** for reliable event publishing (financial-correctness tests: 16). The Wallet Service (`apps/wallet-service`) implements wallet lifecycle, reservations, and optimistic locking, with balance projections synced **event-driven** from the Ledger via Pub/Sub (tests: 36). The Transfer Service (`apps/transfer-service`) orchestrates money movement as a Saga with compensation (tests: 15).
