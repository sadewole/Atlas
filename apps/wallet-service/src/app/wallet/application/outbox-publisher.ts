...
/**
 * The wallet outbox publisher: a background worker that drains
 * `outbox_events` to Pub/Sub. Polls for pending rows, publishes each envelope,
 * then marks the row published.
 *
 * Guarantees:
 * - Events are written to the outbox IN the same DB transaction as the wallet
 *   data, so nothing is lost even if this worker never runs.
 * - Idempotent-safe: Pub/Sub is at-least-once, so consumers dedupe by eventId.
 *   If publishing succeeds but we crash before marking the row, the row is
 *   published again on restart — harmless.
 */
@Injectable()
